...
export default function FormSelectFile() {
    ...
    const onClickButton = () => {
        ...
        if (inputFile.current.length > maxNumFiles) {
    ...
    return (
        <div className="mt-4 mx-auto p-3 rouded"
            style={{ width: '400px', background: '#cee' }}>

            <form>
                <div className="form-group mb-3">
                    <label htmlFor="maxNumFiles" className="form-label">จำนวนไฟล์สูงสุด</label>
                    <select id="maxNumFiles" className="form-select form-select" ref={selectMaxNumFiles}>
                        {
                            maxNumfiles.map(i => {
